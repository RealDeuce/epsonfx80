...
| Printable character gate | `0x0BCF..0x0BFF`; `ESC I` toggles `$8003` bit `0x20` at `0x0BB9..0x0BC6`; `ESC 6/7` toggles `$8003` bit `0x40` at `0x0BC7..0x0BCE` | Determines whether low controls, high controls, and high `DEL` print as glyphs or dispatch as controls. |
| International remap table | `0x0AF7..0x0B7A`, helper at `0x0AC8..0x0AF6` | The table is data, not code. The helper maps selected printable characters through the active international set at `$804F`, matching on the low seven bits and preserving the input high bit. |
| Italic mode and high-half forcing | `ESC 4`/`ESC 5` handlers at `0x0AAC`/`0x0AB0`; text paths at `0x073B..0x0741` and `0x0822..0x0828`; helper at `0x0AB4..0x0ABD` | `ESC 4` sets `$805B` bit `0x10`; `ESC 5` clears it. The text paths call international remap first, then force bit 7 in the internal glyph code when Italic mode is active. High input bytes therefore remain high-half glyphs even in Roman mode. |
| Double-strike/script pass | `ESC G`/`ESC H` handlers at `0x3A79`/`0x3A7D`; resolver at `0x3AF0..0x3B34`; Script handler at `0x3A9D..0x3AAF` | Raw Double-Strike is `$8001` bit `0x10`. The resolver maps effective Double-Strike to `$8000` bit `0x10` and `$8007` bit `0x10`. Script raw bit `$8001.7` sets the same effective pass plus `$8007` bits `0x18`, and masks the separate raw Double-Strike branch until Script is cleared. |
...
| `ESC G` | Double-strike on. | Notes `Escape commands` | Uppercase ESC table `0x09C7`: `G -> 0x3A79`; handler sets raw `$8001` bit `0x10` and calls resolver `0x3AF0` | ROM verified | Double-strike is a second vertically offset pass, not the horizontal emphasized overstrike. |
| `ESC H` | Double-strike off. | Notes `Escape commands` | Uppercase ESC table `0x09C7`: `H -> 0x3A7D`; handler clears raw `$8001` bit `0x10` and calls resolver `0x3AF0` | ROM verified | Does not clear Script; Script has its own raw bit and forces the double-strike/script pass until `ESC T`. |
